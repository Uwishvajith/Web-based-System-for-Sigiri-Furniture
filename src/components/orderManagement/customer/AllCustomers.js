...
export default function AllCustomers() {
    ...
    useEffect(() => {
        ...
        if (document.getElementById('submit').clicked) {//this get executed if we are specifically searching
            ...
            function getCustomers() {
                axios.get("https://sigiri-furniture-app.herokuapp.comcustomer/").then((res) => {
    ...
    function searchCustomers(e) {
        ...
        if (!isNaN(search.charAt(0))) {//checking if the value entered at the search box is for NIC or normal name
            axios.get(`https://sigiri-furniture-app.herokuapp.comcustomer/searchCustomer/${search}`).then((res) => {
            ...
            axios.get(`https://sigiri-furniture-app.herokuapp.comcustomer/searchCustomerByName/${search}`).then((res) => {
                setCustomers(res.data);
            }).catch((error) => {
                alert(error.message);
            })
    ...
    const deleteCustomer = async NIC => {
        ...
        if (answer) {
            ...
            await axios.delete(`https://sigiri-furniture-app.herokuapp.comcustomer/delete/${NIC}`);
