...
export default function AllOrders() {
    ...
    const deleteOrder = async orderId => {
        ...
        if (answer) {
            ...
            await axios.delete(`https://sigiri-furniture-app.herokuapp.comorderItem/deleteOrderItem/${orderId}`)
            alert("OrderItems successfully deleted");

            await axios.delete(`https://sigiri-furniture-app.herokuapp.comorder/deleteOrder/${orderId}`)
    ...
    function searchOrders(e) {
        ...
        if (!isNaN(search.charAt(0))) {//checking if the value entered at the search box is for NIC or normal name
            axios.get(`https://sigiri-furniture-app.herokuapp.comorder/searchOrders/${search}`).then((res) => {
                setOrders(res.data);
            }).catch((error) => {
                alert(error.message);
            })
        } else {
            axios.get(`https://sigiri-furniture-app.herokuapp.comorder/searchOrdersByOrderId/${search}`).then((res) => {
                setOrders(res.data);
            }).catch((error) => {
                alert(error.message);
            })
