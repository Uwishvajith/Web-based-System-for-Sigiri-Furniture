...
export default class EditInventory extends Component {
  ...
  componentDidMount() {
    axios
      .get("http://sigiri-furniture-app.herokuapp.com/inventories/" + this.props.match.params.id)  // get method for editing inventory
      .then(response => {
        this.setState({
      ...
      .catch(function (error) {  // if editing is not succesful show an error
        ...
        axios.get("http://sigiri-furniture-app.herokuapp.com/items/").then(response => {
  ...
  onSubmit(e) {
   ...
   /* if(this.state.category.length <= 3){
    ...
    axios.post("http://sigiri-furniture-app.herokuapp.com/inventories/update/" + this.props.match.params.id,inventory)
