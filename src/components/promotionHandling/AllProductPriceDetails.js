...
export default function AllProductPriceDetails() {
  ...
  const deleteProductPrice = async salesid => {
    ...
    if (answer) {
      ...
      await axios.delete(`https://sigiri-furniture-app.herokuapp.comproductprice/deletedelete/${salesid}`);
  ...
  function searchProductPriceID(e) {
    e.preventDefault();
    axios.get(`https://sigiri-furniture-app.herokuapp.comproductprice/searchProductByID/${searchsaleid}`).then((res) => {
      console.log(res.data);
      setProductPrices(res.data);
    }).catch((err) => {
      alert(err.message);
